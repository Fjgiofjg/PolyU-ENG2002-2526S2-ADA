...
def _round_div(numer: int, denom: int) -> int:
    """Fixed-point numbers — `fixNum` implementation and simple CLI menu.

    This module provides the `fixNum` class representing a fixed-point number
    `a.b` with a global precision `prec`. It implements addition (Task 1)
    and the distinction `power` functionality.
    """
...
class fixNum:
    def __init__(self, a: int = 0, b: int = 0, prec_local: Optional[int] = None):
    ...
    @classmethod
    def from_scaled(cls, scaled: int, prec_local: Optional[int] = None) -> "fixNum":
    ...
    def _power_int(self, n: int) -> "fixNum":
        if n == 0:
            return fixNum.from_scaled(self.scale, prec_local=self.prec)
        p = self._scaled
        q = self.scale
        if n > 0:
            p_abs = abs(p)
...
def _read_fixnum(prompt: str = "") -> fixNum:
    tag = (" " + prompt) if prompt else ""
    while True:
        try:
            a = int(input(f"Enter integer part a{tag}: ").strip())
            b_raw = input(f"Enter fractional part b{tag} (digits only, e.g. 48 for .48): ").strip()
            if b_raw == "":
                b = 0
            else:
                b = int(b_raw)
...
def menu() -> None:
    print("Fixed-point calculator (Task 1 + Distinction)")
    print(f"Precision: {prec} decimal places")
    while True:
        print("\nMenu:\n1) Add two fixed-point numbers\n2) Power (integer or fixed-point exponent)\n3) Exit")
        choice = input("Choice: ").strip()
        if choice == "1":
            x = _read_fixnum("(first)")
            y = _read_fixnum("(second)")
            r = x.add(y)
            print(f"{x} + {y} = {r}")
        elif choice == "2":
            base = _read_fixnum("(base)")
            t = input("Exponent type - enter 'i' for integer, 'f' for fixed-point: ").strip().lower()
            if t == "i":
                try:
                    n = int(input("Enter integer exponent n: ").strip())
                    res = base.power(n)
                    print(f"{base} ** {n} = {res}")
                except Exception as e:
                    print("Error:", e)
            elif t == "f":
                exp = _read_fixnum("(exponent)")
                try:
                    res = base.power(exp)
